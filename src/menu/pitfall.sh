...
oneshot() {
	...
	if anykey "A2600 - Pitfall! - Pitfall Harry's Jungle Adventure.pdf" ; then
		if [ "$got" = "n" ]; then
			qstart -L stella "$RA/roms/Atari 2600/Pitfall! - Pitfall Harry's Jungle Adventure (USA).zip"
		else
			cheevos -L stella "$RA/roms/Atari 2600/Pitfall! - Pitfall Harry's Jungle Adventure (USA).zip"
		fi
	fi
}
